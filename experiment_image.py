...
def spawn_vehicle_rear(world, blueprint):
    map = world.get_map()
    origin_loc = carla.Location(x=205.74, y=11.27, z=0.3)
...
def spawn_vehicle_front(world, blueprint):
    map = world.get_map()
    origin_loc = carla.Location(x=145.74, y=10.46, z=0.3)
    base_wp = map.get_waypoint(origin_loc, project_to_road=True, lane_type=carla.LaneType.Driving)
...
def main():
    ...
    try:
        ...
        front_vehicle = spawn_vehicle_front(world, vehicle_bp)
        rear_vehicle = spawn_vehicle_rear(world, vehicle_bp)
